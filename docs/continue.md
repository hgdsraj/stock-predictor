...
| 16 | INFRA DONE; SWEEP RUNNING | TBD | TBD | Chain sweep driver `scripts/phase16_chain_sweep.py`: 4 configs on Phase 13 baseline (baseline, +TB, +conf, +TB+conf) to test whether triple-barrier labels and/or confidence sizing improve on Phase 13's binary meta. Production sweep is in-flight (CPU contention with GDELT bulk; expected ~30 min total once GDELT finishes). |
...
| 19 | INFRA DONE; SWEEP DRIVER READY | TBD (production sweep pending) | TBD | Per-ticker Bayesian shrinkage of ensemble score by historical sign-precision. Formula: shrink_t = clip((p_t - 0.5)/0.5, 0, 1) * alpha. New module `src/stockpred/portfolio/bayesian_shrinkage.py`. Pipeline integration via `bayesian_shrinkage_alpha` config. **CRITICAL fix from reviewer C1**: factors fit on first 80% of dev are now applied ONLY to holdout (not dev), so dev metrics stay clean OOS. H1 fix: multi-horizon avg realised return (not just shortest). H4 fix: explicit index-name assertion. 14 tests passing. Sweep driver: `scripts/phase19_shrinkage_sweep.py`. |
