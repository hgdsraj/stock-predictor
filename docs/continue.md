...
| 14 | DONE (bulk fetch) | TBD (production smoke running) | TBD | GDELT 1.0 daily tone + mentions. Bulk fetch complete: 4018 daily files in 3 hr 54 min (0.3 files/sec at 0.5s rate limit). 3922 days with data, 6 empty days, 90 errors (2.2% mostly transient network blips). Cache directory: 55 MB on disk. Per-day parquet ~10-100 KB after S&P-500 filter (≈100x compression vs raw zips). New module `src/stockpred/data/gdelt.py` (streaming CSV parser). 6 features per (date, ticker): mention_count, article_count, tone_mean, tone_std, rolling 5d/21d. CLI `--gdelt`, pipeline field `use_gdelt_features`, HTTP schema field too. 11 unit tests passing. Phase 13+GDELT production smoke launched. |
