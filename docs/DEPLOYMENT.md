...
#### Best honest config (Phase 13)

See [`docs/OPTIMAL.md`](OPTIMAL.md) for the canonical curl command,
complete parameter list, and honest expectations (Sharpe +0.17, CI
[−0.32, +0.58], DD −8.2%). The short version: `use_edgar_item_features:
true`; do NOT add `use_edgar_features: true` (raw 8-K counts hurt).

Set `EDGAR_USER_AGENT="Your Name your-email@example.com"` on the server
(SEC compliance). Returns `409` if a pipeline run is already in flight.
